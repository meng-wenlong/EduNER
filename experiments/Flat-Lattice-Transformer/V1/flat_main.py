...
parser.add_argument('--save_path', type=str, default="result/save")
...
if args.status == "test":
    model_dir = "./{}/{}/2021_12_26_01_44_56".format(args.save_path, args.dataset)
    model_path = "{}/model_ckpt.pkl".format(model_dir)
    ...
    def num2label(i: int):
        return vocabs['label'].idx2word[i]

    gold_results = datasets['test']['target']
    gold_tags = []
    for gold_label in gold_results:
        item = list(map(num2label, gold_label))
        gold_tags.append(item)

    predictor = Predictor(model)
    predictor.batch_size = 1

    test_label_list = predictor.predict(datasets['test'])['pred']
    pred_tags = []
    for test_label in test_label_list:
        for item_x in test_label:
            item_y = list(map(num2label, item_x))
            pred_tags.append(item_y)
             

    classification_report_res = classification_report(y_pred=pred_tags, y_true=gold_tags, digits=4)
    print(classification_report_res)
    with open('result/classification_report1226.txt', 'w', encoding='utf-8') as f:
        f.write(classification_report_res)
        f.close()
