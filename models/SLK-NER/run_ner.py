...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        checkpoints = [os.path.join(args.output_dir, "best_bertCrf_model_7770_0.6527")]
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        model = model_class.from_pretrained(os.path.join(args.output_dir, "best_bertCrf_model_7770_0.6527"), config=config, label2id=args.label2id,
                                            device=args.device, data=data)

        model.to(args.device)
        result, predictions = evaluate(args, data, model, tokenizer, labels, pad_token_label_id, mode="test")
        # Save results
