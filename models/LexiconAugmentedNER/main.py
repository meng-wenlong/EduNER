...
def evaluate(data, model, name):
    ...
    for batch_id in range(total_batch):
        with torch.no_grad():
            ...
            gaz_list = [
                data.gaz_alphabet.get_instance(id) for batchlist in gaz_match if len(batchlist) > 0 for id in batchlist
            ]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', default="DisciplineNER/train.char.bmes")
    parser.add_argument('--dev', default="DisciplineNER/dev.char.bmes")
    parser.add_argument('--test', default="DisciplineNER/test.char.bmes")
    ...
    if status == 'train':
        ...
        data.HP_use_count = args.use_count
        data.generate_instance_with_gaz(test_file, 'test')
        pred = load_model_decode(save_model_dir, data, 'test', gpu, seg)
        with open('result/pred_result12.txt', 'w', encoding='utf-8') as w:
            w.write(str(pred))
            w.close()
